...
const Calculator = React.memo((checkBox) => {
  ...
  const onDelete = async (index) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_JSON}/checkedItems/${itemToDelete.id}`
      );
  ...
  const enoughEmoji =
    kcalTotal > parseInt(kcal) ? (
      <span className="emoji1">
        {isMobile ? (
          <FeatherIcon icon="frown" size="40" />
        ) : (
      ...
      <span className="emoji2">
        {isMobile ? (
          <FeatherIcon icon="smile" size="40" />
        ) : (
    ...
    <>
              ...
              return (
                <li key={index} className="panel">
                  ...
                  <p className="liBtn" onClick={() => onDelete(index)}>
                    <Icon className="icon" icon="ion:close" width="24px" />
                  </p>
                </li>
              );
            })
      ...
      <div>
        <button className="resultBtn" onClick={handleResult}>
          <FeatherIcon icon="search" size="40" stroke="#fff" />
        </button>
      </div>
    </>
  );
